...
export default {
    ...
    exec: async ({ conn, m, command, usedPrefix }) => {
        ...
        try {
            // 2. Pilih Random URL & Download Buffer
            const randomUrl = gachaLinks[Math.floor(Math.random() * gachaLinks.length)];
            const imageBuffer = await (await fetch(randomUrl)).buffer();
            
            // 3. Prepare Media buat Header
            const media = await prepareWAMessageMedia({ image: imageBuffer }, { upload: conn.waUploadToServer });

            // 4. Struktur Interactive Message Loli
            const interactiveMessage = {
                ...
                body: { 
                    text: "Satu loli imut berhasil didapatkan! Simpan baik-baik , jangan sampai ketahuan pihak berwajib ya~ 😭✨" 
                },
                footer: { 
                    text: global.footer 
                },
            ...
            console.error('Loli Gacha Error:', e);
            await m.react('❌');
            return conn.sendMessage(m.chat, { 
                image: { url: global.anime },
                caption: `❏ K E S A L A H A N  S Y S T E M ❏\n Alasan: ❌ gacha-nya ampas/eror. Coba lagi ya!` 
            }, { quoted: fail });
        }
    }
};