    ...
    exec: async ({ conn, m, command, usedPrefix }) => {
        ...
        if (!mime || !/image\/(jpe?g|png|webp)/.test(mime)) {
            ...
            return conn.sendMessage(m.chat, { 
                ...
                caption: `⚠️ Silahkan reply gambar yang mau dijadiin Maid ` 
            }, { quoted: m });
        }
        ...
        try {
            ...
            // Hit API FAA - ToMaid
            const apiUrl = `https://api-faa.my.id/faa/tomaid?url=${encodeURIComponent(catboxUrl)}`;
            ...
            await conn.sendMessage(m.chat, {
                image: resultBuffer,
                caption: "✨ Berhasil mengubah ke gaya Maid."
            });

            m.react('✅');

        } catch (e) {
            console.error('ToMaid Error:', e);
            m.react('❌');
