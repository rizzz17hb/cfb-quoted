    ...
    exec: async ({ conn, m, command, usedPrefix }) => {
        ...
        if (!mime || !/image\/(jpe?g|png|webp)/.test(mime)) {
            ...
            return conn.sendMessage(m.chat, { 
                ...
                caption: `⚠️ Silahkan reply gambar yang mau dijadiin kang korupsi ` 
            }, { quoted: m });
        }
        ...
        try {
            ...
            // Hit API FAA - ToDPR
            const apiUrl = `https://api-faa.my.id/faa/todpr?url=${encodeURIComponent(catboxUrl)}`;
            ...
            await conn.sendMessage(m.chat, {
                image: resultBuffer,
                caption: "✨ Berhasil mengubah ke gaya DPR."
            });

            m.react('✅');

        } catch (e) {
            console.error('ToDPR Error:', e);
            m.react('❌');
