    ...
    exec: async ({ conn, m, command, usedPrefix }) => {
        ...
        if (!mime || !/image\/(jpe?g|png|webp)/.test(mime)) {
            ...
            return conn.sendMessage(m.chat, { 
                ...
                caption: `⚠️ Silahkan reply gambar yang mau dijadiin berkacamata ` 
            }, { quoted: m });
        }
        ...
        try {
            ...
            // Hit API FAA - ToKacamata
            const apiUrl = `https://api-faa.my.id/faa/tokacamata?url=${encodeURIComponent(catboxUrl)}`;
            ...
            await conn.sendMessage(m.chat, {
                image: resultBuffer,
                caption: "✨ Berhasil menambahkan kacamata."
            });

            m.react('✅');

        } catch (e) {
            console.error('ToKacamata Error:', e);
            m.react('❌');
