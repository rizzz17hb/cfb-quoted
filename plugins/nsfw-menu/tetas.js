...
export default {
    name: 'tetas',
    alias: ['oppai', 'boobs', 'toket'],
    category: 'nsfw',
    isPremium: true,
    exec: async ({ conn, m }) => {
        try {
            // 1. React Awal (Gaya m.react yang pasti jalan di base lo)
            await m.react('⏱️');

            // 2. Ambil data cadangan (15) untuk filter link mati
            const { data } = await axios.get(`https://api.lolicon.app/setu/v2?num=15&r18=1&tag=oppai&size=regular`);
            ...
            if (valid.length === 0) throw "Semua stok Oppai sedang gangguan (404).";
            ...
            const caption = `✨ *OPPAI PREMIUM ALBUM* ✨\n` +
                            `──────────────────────────\n\n` +
                            `🍒 *Type:* Oppai / Big Boobs\n` +
