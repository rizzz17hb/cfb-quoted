...
export default {
    name: 'booty',
    alias: ['pantat', 'ass', 'butt'],
    category: 'nsfw',
    isPremium: true,
    exec: async ({ conn, m }) => {
        try {
            // 1. React Awal (Gaya m.react yang paling stabil di base lo)
            await m.react('⏱️');

            // 2. Ambil data cadangan (15) untuk filter link mati
            const { data } = await axios.get(`https://api.lolicon.app/setu/v2?num=15&r18=1&tag=ass&size=regular`);
            
            let valid = [];
            // 3. Validasi & Download ke Buffer (Anti-Invalid Media Type)
            ...
            if (valid.length === 0) throw "Semua stok Booty sedang gangguan (404).";
            ...
            const caption = `✨ *BOOTY PREMIUM ALBUM* ✨\n` +
                            `──────────────────────────\n\n` +
                            `🍑 *Type:* Booty / Sexy Ass\n` +
