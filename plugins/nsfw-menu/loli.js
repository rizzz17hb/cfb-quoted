...
export default {
    name: 'loli',
    alias: ['lolinsfw', 'nsfwloli'],
    category: 'nsfw',
    isPremium: true,
    exec: async ({ conn, m }) => {
        try {
            // 1. React Awal (Sesuai base bot lo yang jalan)
            await m.react('⏱️');

            // 2. Ambil cadangan data (15)
            const { data } = await axios.get(`https://api.lolicon.app/setu/v2?num=15&r18=1&tag=loli&size=regular`);
            
            let valid = [];
            // 3. Validasi & Download (Anti-Invalid Media Type)
            ...
            if (valid.length === 0) throw "Semua stok Loli sedang gangguan (404).";

            // 4. Metadata Rapi dengan Jeda \n\n
            ...
            const caption = `✨ *LOLI PREMIUM ALBUM* ✨\n` +
                            `──────────────────────────\n\n` +
                            `🎒 *Type:* Loli / Cute Core\n` +
