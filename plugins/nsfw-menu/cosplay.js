...
export default {
    name: 'cosplay',
    alias: ['hentaicosplay', 'cosplaynsfw'],
    category: 'nsfw',
    isPremium: true,
    exec: async ({ conn, m }) => {
        try {
            // 1. React sesuai base bot lo
            await m.react('⏱️');

            // 2. Ambil random dari database global lo
            let url = global.cosplay[Math.floor(Math.random() * global.cosplay.length)];

            // 3. Download ke Buffer (Logic yang sama kayak Pixiv tadi)
            ...
            // 4. Caption Sultan
            const caption = `✨ *HENTAI COSPLAY PREMIUM* ✨\n` +
                            `──────────────────────────\n\n` +
                            `🔞 *Type:* Cosplay / Real Life\n` +
                            `📸 *Source:* Internal Database\n` +
                            `🔒 *Access:* High-Level Member\n\n` +
                            `──────────────────────────\n` +
                            `_Nikmati koleksi eksklusif anda..._`;

            // 5. Kirim pake sendMessage biar sinkron sama Mimetype
            ...
            await m.react('❌');
            await conn.sendMessage(m.chat, { 
                caption: `❌ *SYSTEM ERROR*\n\nReason: Link mati atau server sedang sibuk.` 
            }, { quoted: m });
        }
    }
};

// Database global.cosplay lo taruh di sini atau di file global.js
global.cosplay = [
