...
export default {
    ...
    exec: async ({ conn, m }) => {
        try {
            ...
            const { data } = await axios.get(`https://api.lolicon.app/setu/v2?num=15&r18=1&tag=yuri&size=regular`);
            ...
            if (valid.length === 0) throw "Semua stok Yuri sedang gangguan (404).";
            ...
            const caption = `✨ *YURI PREMIUM ALBUM* ✨\n` +
                            `──────────────────────────\n\n` +
                            `👭 *Type:* Yuri / Lesbian\n` +
