...
export default {
    ...
    exec: async ({ conn, m }) => {
        try {
            // 1. React Loading (Pake m.react sesuai base lo)
            await m.react('⏱️');

            const { data } = await axios.get(`https://api.lolicon.app/setu/v2?num=15&r18=1&tag=ecchi&size=regular`);
            ...
            if (valid.length === 0) throw "Semua stok Ecchi sedang gangguan (404).";
            ...
            const caption = `✨ *ECCHI PREMIUM ALBUM* ✨\n` +
                            `──────────────────────────\n\n` +
                            `🔞 *Type:* Ecchi / Soft Core\n` +
